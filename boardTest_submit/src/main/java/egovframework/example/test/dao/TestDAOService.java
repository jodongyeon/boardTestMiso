...
public class TestDAOService implements TestDAO {
	...
	/*SqlSession 객체를 통해 interface TestMappler에 접근, testMapper 에 작성해 놓은 SQL문을 실행 할 수 있음
...
// testmapper namespace에서 1번 방식으로 mapper를 지정하지 않았다면 이렇게 namespace의 이름으로 연결해서SQL문을 실행시키겠지	
/*	public List<TestVO> getBoardList(Pagination pagination) throws Exception{
//		return sqlSession.selectList("egovframework.sqlmap.mappers.testMapper.getBoardListCnt", pagination);
